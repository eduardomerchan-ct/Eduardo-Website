...
void * thread_c(void * arg_in)
{
    ...
    pthread_mutex_lock(&arg->mutex);//this locks the mutex
    arg->max=gmn_get_max();
    arg->status=S_MAX;
...
void * thread_p(void *arg_in)
{
    ...
    check_pthread_return(pthread_mutex_lock(&arg->mutex),"lock failed");
    while (arg->status!=S_MAX){
        pthread_cond_wait(&arg->cond_result, &arg->mutex);

    }
    max=arg->max;
    check_pthread_return(pthread_mutex_unlock(&arg->mutex), "unlock failed");
...
int main(int argc, char *argv[])
{
    ...
    check_pthread_return(pthread_create(&tc,NULL, thread_c, &arg),"thread c failed");
    check_pthread_return(pthread_create(&tp,NULL, thread_p,&arg),"thread p failed");
    check_pthread_return(pthread_join(tc,NULL),"join c failed");
    check_pthread_return(pthread_join(tp,NULL),"join p has failed");
